...
public class SmartCrawler extends JFrame
{
	...
	public SmartCrawler()
	{
		...
		setTitle("SmartCrawler::SmartCrawler A Two stage Crawler for Efficiently Harvesting Deep Web Interfaces");
		...
		JMenuItem View_Ranking = new JMenuItem("View Site Ranking", KeyEvent.VK_R);
		...
		View_Ranking.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent e)
			{
				View_SiteCrawler_Details vsd=new View_SiteCrawler_Details();
				vsd.setSize(600,500);
				vsd.setVisible(true);
			}
		});
		...
		JLabel startLabel = new JLabel("Enter Site URL:");
		...
		searchButton.addActionListener(new ActionListener()
		{
			public void actionPerformed(ActionEvent e)
			{
				...
				// Write Ranking here
	             ...
	             String surl=startTextField.getText();
	             String murl=String.valueOf(maxComboBox.getSelectedItem());//matchesLabel2.getText();
	             
	             
	             ResultSet rs=stmt.executeQuery("select * from Crawler where urlname='"+surl+"' and sword='"+searchword+"'");
	             
	             if(rs.next()==true)
	             {
	            
	            	 String r=rs.getString(4);
	            	 int cr=Integer.parseInt(r)+1;
	            	 
	            	 String query11 = "update Crawler set rank='"+String.valueOf(cr)+"' where urlname='"+surl+"' and sword='"+searchword+"'";
	             ...
	             else if(rs.next()!=true)
	             {
	            	 ...
	            	 String query1 = "insert into Crawler(urlname,sword,rank,murl,dt) values('"+surl+"','"+searchword+"','"+rank+"','"+murl+"','"+dt+"')";
		...
		JLabel toCrawlLabel1 = new JLabel("Time To Crawl:");
		constraints = new GridBagConstraints();
		constraints.anchor = GridBagConstraints.EAST;
		constraints.insets = new Insets(5, 5, 0, 0);
	...
	private void search(final String logFile, final String startUrl, final int maxUrls, final String searchString)
	{
		...
		Thread thread = new Thread(new Runnable()
		{
			...
			public void run()
			{
				...
				{
					JOptionPane.showMessageDialog(SmartCrawler.this,
	...
	private void updateStats(String crawling, int crawled, int toCrawl, int maxUrls)
	{
		...
		System.out.println("Time Taken="+st1);
...
/*
 * Character Sequence Explanation <a Look for the characters "<a". \\s+ Look
 * for one or more space characters. href Look for the characters "href". \\s*
 * Look for zero or more space characters. = Look for the character "--". \\s*
 * Look for zero or more space characters. \"? Look for zero or one quote
 * character. (.*?)Look for zero or more of any character until the next part of
 * the pattern is matched, and place the results in a group. [\">]Look for quote
 * character or greater than (">") character.
 */
